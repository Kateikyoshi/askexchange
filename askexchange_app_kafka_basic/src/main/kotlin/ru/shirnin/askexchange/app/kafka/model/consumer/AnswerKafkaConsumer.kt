...
import ru.shirnin.askexchange.app.kafka.strategies.answer.AnswerConsumerStrategy
...
class InnerAnswerProcessor {
    fun exec(ctx: InnerAnswerContext) {
        ctx.answerResponse = InnerAnswerStub.get()
    }
}

class AnswerKafkaConsumer(
    private val config: AppKafkaConfig,
    answerConsumerStrategies: List<AnswerConsumerStrategy>,
    private val processor: InnerAnswerProcessor = InnerAnswerProcessor(),
    private val consumer: Consumer<String, String> = config.createKafkaConsumer(),
    private val producer: Producer<String, String> = config.createKafkaProducer()
) {
    private val process = atomic(true)
    private val topicsAndStrategyByInputTopic = answerConsumerStrategies.associate {
    ...
    private fun sendResponse(context: InnerAnswerContext, strategy: AnswerConsumerStrategy, outputTopic: String) {
    ...
    private data class TopicsAndStrategy(val inputTopic: String, val outputTopic: String, val answerStrategy: AnswerConsumerStrategy)
}